...
{
    public partial class UctDisc : UserControl, IUctShape
    {
        public UctDisc()
        {
            InitializeComponent();
            ...
            LblCount.MouseEnter += Tooltip_CirclesInACircle_MouseEnter;
            LblCount.MouseLeave += Tooltip_MouseLeave;

            foreach (var count in Packing.CirclesInACircle.Table.Keys)
            {
                CmbCount.Items.Add(count);
            }
            CmbCount.SelectedIndex = CmbCount.Items.IndexOf(127);
        }
        ...
        public override string ToString() => $"Disc vertices:{Write(true, true, true)} weight={PnlWeight.Tag}, x={NudX.ToDouble()}, y={NudY.ToDouble()}, z={NudZ.ToDouble()}, r={NudR.ToDouble()}, count={CmbCount.Text}{(ChbRotate.Checked ? Urt : "")}{(ChbCloneVibrate.Checked ? Ucv : "")}{(ChbOrbitVibrate.Checked ? Uov : "")}";
        ...
        private void Tooltip_CirclesInACircle_MouseEnter(object? sender, EventArgs e)
        {
            if (sender is IWin32Window window)
            {
                Tlt.Show("from packing 'circles in a circle'", window);
            }
        }
