...
{
    public partial class UctPolygonArea : UserControl, IUctShape
    {
        public UctPolygonArea()
        {
            InitializeComponent();
            SetWeight(16);

            LblPointsCount.MouseEnter += Tooltip_CirclesInASquare_MouseEnter;
            LblPointsCount.MouseLeave += Tooltip_MouseLeave;

            foreach (var count in Packing.CirclesInASquare.Table.Keys)
            {
                CmbPointsCount.Items.Add(count);
            }
            CmbPointsCount.SelectedIndex = CmbPointsCount.Items.IndexOf(450);
        }
        ...
        public override string ToString() => $"PolygonArea vertices:{Write(true, true, true)} weight={PnlWeight.Tag}, x={NudX.ToDouble()}, y={NudY.ToDouble()}, z={NudZ.ToDouble()}{(ChbRotate.Checked ? Urt : "")}, r={NudR.ToDouble()}, count={NudEdgesCount.ToInt()}{(ChbCloneVibrate.Checked ? Ucv : "")}{(ChbOrbitVibrate.Checked ? Uov : "")}";
        ...
        private void Tooltip_CirclesInASquare_MouseEnter(object? sender, EventArgs e)
        {
            if (sender is IWin32Window window)
            {
                Tlt.Show("from packing 'circles in a square'", window);
            }
        }
        ...
        public IUctShape Clone()
        {
            ...
            ret.CmbPointsCount.Text = CmbPointsCount.Text;
        ...
        {
            return PolygonAreaSetting.Get(this);
        }
        #endregion
    }
}
