...
{
    public partial class UctRectangularArea : UserControl, IUctShape
    {
        public UctRectangularArea()
        {
            InitializeComponent();
            SetWeight(16);

            LblCount.MouseEnter += Tooltip_CirclesInARectangle_MouseEnter;
            LblCount.MouseLeave += Tooltip_MouseLeave;

            foreach (var count in Packing.CirclesInARectangle.Table.Keys)
            {
                CmbCount.Items.Add(count);
            }

            CmbCount.SelectedIndex = CmbCount.Items.IndexOf(500);
        }
        ...
        public override string ToString() => $"Rectangular area vertices:{Write(true, true, true)} weight={PnlWeight.Tag}, x={NudX.ToDouble()}, y={NudY.ToDouble()}, z={NudZ.ToDouble()}, width={NudWidth.ToDouble()}, height={NudHeight.ToDouble()}, count={CmbCount.Text}{(ChbRotate.Checked ? Urt : "")}{(ChbCloneVibrate.Checked ? Ucv : "")}{(ChbOrbitVibrate.Checked ? Uov : "")}";
        ...
        private void Tooltip_CirclesInARectangle_MouseEnter(object? sender, EventArgs e)
        {
            if (sender is IWin32Window window)
            {
                Tlt.Show("from packing 'circles in a rectangle'", window);
            }
        }
        ...
        {
            var ret = new UctRectangularArea();
        ...
        {
            return RectangularAreaSetting.Get(this);
        }
        #endregion
    }
}
