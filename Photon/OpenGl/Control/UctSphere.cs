...
{
    public partial class UctSphere : UserControl, IUctShape
    {
        public UctSphere()
        {
            InitializeComponent();
            ...
            LblCount.MouseEnter += Tooltip_SpheresInASphere_MouseEnter;
            LblCount.MouseLeave += Tooltip_MouseLeave;

            foreach (var count in Packing.SpheresInASphere.Table.Keys)
            {
                CmbCount.Items.Add(count);
            }

            CmbCount.SelectedIndex = CmbCount.Items.IndexOf(180);
        }
        ...
        public override string ToString() => $"Sphere vertices:{Write(true, true, true)} weight={PnlWeight.Tag}, x={NudX.ToDouble()}, y={NudY.ToDouble()}, z={NudZ.ToDouble()}, r={NudR.ToDouble()}, count={CmbCount.Text}{(ChbRotate.Checked ? Urt : "")}{(ChbCloneVibrate.Checked ? Ucv : "")}{(ChbOrbitVibrate.Checked ? Uov : "")}";
        ...
        private void Tooltip_SpheresInASphere_MouseEnter(object? sender, EventArgs e)
        {
            if (sender is IWin32Window window)
            {
                Tlt.Show("from packing 'spheres in a sphere'", window);
            }
        }
