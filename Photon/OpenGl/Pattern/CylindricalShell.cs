...
{
    public class CylindricalShell : Shape
    {
        public CylindricalShell(double x, double y, double z, double r, double length, int countSpheresInCube, Vec4 color, double pointSize = 1)
        {
            ...
            var rMin = r * ratio - row.Radius;
            var rMax = r * ratio + row.Radius;
            ...
            foreach (var c in cs)
            {
                if (Compare.In(c.Z, l1, l2) && Compare.In(Math.Sqrt(c.X * c.X + c.Y * c.Y), rMin, rMax))
                {
                    var cx = x + c.X / ratio;
                    var cy = y + c.Y / ratio;
