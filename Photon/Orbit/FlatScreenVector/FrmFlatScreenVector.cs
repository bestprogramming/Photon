...
{
    public partial class FrmFlatScreenVector : Form
    {
        public readonly string Header;

        public FrmFlatScreenVector()
        {
            InitializeComponent();
            Header = Text;
        }

        private void FrmFlatScreenVector_Load(object sender, EventArgs e)
        {
        ...
        private void Smi_Click(object sender, EventArgs e)
        {
            if (sender == SmiSave)
            {
                FormSetting.Write(FormSetting.Get(this), $"{nameof(FrmFlatScreenVector)}.xml");
            }
            else if (sender == SmiLoad)
            {
                var setting = FormSetting.Read($"{nameof(FrmFlatScreenVector)}.xml");
        ...
        private void RdbScreenPlane_CheckedChanged(object sender, EventArgs e)
        {
            var rdb = (RadioButton)sender;
            if (rdb.Checked)
            {
                FlpScreenPlane.Tag = rdb.Tag;
            }
        }
        ...
        private static void RenderFile(FormSetting setting, string dir, bool openRender = true)
        {
            ...
            {
                var directory = $"Renders-FlatScreenVector/{dir}";
                Directory.CreateDirectory(directory);
                FormSetting.Write(setting, $"{directory}/{nameof(FrmFlatScreenVector)}.xml");
