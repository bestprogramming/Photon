...
namespace Photon.Space
{
    public partial class FrmSpace : Form
    {
        ...
        private void Smi_Click(object sender, EventArgs e)
        {
            if (sender == SmiSave)
            {
                FormSetting.Write(FormSetting.Get(this), $"{nameof(FrmSpace)}.xml");
            }
            else if (sender == SmiLoad)
            {
                var setting = FormSetting.Read($"{nameof(FrmSpace)}.xml");
        ...
        private static void RenderFile(FormSetting setting, string dir, bool openRender = true)
        {
            ...
            {
                var directory = $"Renders-Space/{dir}";
                Directory.CreateDirectory(directory);
                FormSetting.Write(setting, $"{directory}/{nameof(FrmSpace)}.xml");
