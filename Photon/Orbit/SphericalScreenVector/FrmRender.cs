...
namespace Photon.SphericalScreenVector
{
    public partial class FrmRender : Form
    {
        ...
        public Vector3 GetVector(in Orbit oA, in Orbit oB, in Point3 p)
        {
            Dimension.Lerp(p.X, p.Y, p.Z, oA.X, oA.Y, oA.Z, oA.W / (oA.N * oA.N), out Vector3 f1);
            Dimension.Lerp(p.X, p.Y, p.Z, oB.X, oB.Y, oB.Z, oB.W / (oB.N * oB.N), out Vector3 f2);
            var ft = f1 + f2;
            var normal = new Vector3(setting.Screen.X - p.X, setting.Screen.Y - p.Y, setting.Screen.Z - p.Z);
            return ft * normal * normal.Unit() / normal.Length();
        }
                    ...
                    foreach (var oA in osA)
                    {
                        foreach (var oB in osB)
                        {
                            ...
                            if (oA.Cp.Intersect(oB.Cp, out Point3 p1, out Point3 p2))
                            {
                                ...
                                if (PointInRange(p1))
                                {
                                    var v = GetVector(oA, oB, p1);
                                    addToDeep(deeps, GetPixel3(p1, setting), v.Length());
                                }

                                if (PointInRange(p2))
                                {
                                    var v = GetVector(oA, oB, p2);
                                    addToDeep(deeps, GetPixel3(p2, setting), v.Length());
                                }
                            }
                        }
                    }
